...
<div style="max-width: 600px; margin: 0 auto;">
    <div style="margin-bottom: 24px;">
        <h1>Edit Address</h1>
        <a href="{{ route('account.addresses') }}" style="color: #007bff; text-decoration: none;">← Back to Addresses</a>
    </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('account.addresses.update', $address->id) }}">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input id="name" type="text" name="name" value="{{ old('name', $address->name) }}" required
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <textarea id="address" name="address" rows="3" required
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">{{ old('address', $address->address) }}</textarea>
                    @error('address')
                        <div style="color: #dc3545; font-size: 0.875em; margin-top: 5px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input id="city" type="text" name="city" value="{{ old('city', $address->city) }}" required
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                    @error('city')
                        <div style="color: #dc3545; font-size: 0.875em; margin-top: 5px;">{{ $message }}</div>
                    @enderror
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input id="state" type="text" name="state" value="{{ old('state', $address->state) }}" required
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                    @error('state')
                        <div style="color: #dc3545; font-size: 0.875em; margin-top: 5px;">{{ $message }}</div>
                    @enderror
                ...
                <div style="margin-bottom: 15px;">
                    <label for="pincode" style="display: block; margin-bottom: 5px; font-weight: bold;">Pincode *</label>
                    <input id="pincode" type="text" name="pincode" value="{{ old('pincode', $address->pincode) }}" required
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                    @error('pincode')
                        <div style="color: #dc3545; font-size: 0.875em; margin-top: 5px;">{{ $message }}</div>
                    @enderror
                ...
                <div style="display: flex; gap: 10px;">
                    ...
                    <a href="{{ route('account.addresses') }}" class="btn" style="background: #f8f9fa;">Cancel</a>
                </div>
            </form>
        </div>
    </div>
